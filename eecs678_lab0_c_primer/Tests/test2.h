...
int test2() {
    printf("Running Test 2: Linked List Functions\n");
    int points = 10;

    // Step 1: Create two sorted linked lists of different lengths
    ...
    if (mergedList == NULL) {
        ...
        printf("Merged List: ");
        printList(mergedList);  // Expected output: "1 -> 2 -> 3 -> 4 -> 5 -> 7 -> 9 -> NULL"
        ...
        expected = insertAtEnd(expected, 9);
